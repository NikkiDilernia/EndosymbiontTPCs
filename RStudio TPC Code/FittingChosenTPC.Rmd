...
Fitting chosen TPC (e.g., Quadratic algorithm) to data
---
...
```{r}
start_vals<- get_start_vals(data2$Temp, data2$FVFM, model_name = 'quadratic_2008')

low_lims <- get_lower_lims(data2$Temp, data2$FVFM, model_name = 'quadratic_2008')
upper_lims <- get_upper_lims(data2$Temp, data2$FVFM, model_name = 'quadratic_2008')
...
```{r}
fit <- nls_multstart(FVFM~quadratic_2008(temp = Temp, a, b, c),
                                                     data = data2,
                                                     iter = c(3), 
...
ggplot(aes(Temp, FVFM)) +
  ...
  ylim(0, 0.65) +
  xlim(17, 35) +
  geom_line(aes(Temp, preds), colour="#56B4E9", size = 2) +
  theme(
